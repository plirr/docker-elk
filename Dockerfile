FROM java:8
MAINTAINER Gordon Goldner <plirr@hotmail.com>
...
RUN \
    ...
    if ! grep "elasticsearch" /etc/apt/sources.list; then echo "deb http://packages.elasticsearch.org/elasticsearch/6.3/debian stable main" >> /etc/apt/sources.list;fi && \
    if ! grep "logstash" /etc/apt/sources.list; then echo "deb http://packages.elasticsearch.org/logstash/6.3/debian stable main" >> /etc/apt/sources.list;fi && \
    apt-get update && \
...
# Kibana
RUN \
    curl -s https://download.elasticsearch.org/kibana/kibana/kibana-6.3.0-linux-x64.tar.gz | tar -C /opt -xz && \
    ln -s /opt/kibana-6.3.0-linux-x64 /opt/kibana && \
